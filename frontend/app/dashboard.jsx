...
export default function UserDashboard() {
  ...
  const fetchUserData = async () => {
    try {
      ...
      const response = await fetch(
        `${API_BASE_URL}/user/${encodeURIComponent(userEmail)}`,
      );
      const result = await response.json();
  ...
  const handleQueueRefresh = async () => {
    if (!userId) return;
    try {
      const response = await fetch(`${API_BASE_URL}/queue/refresh/${userId}`, {
        method: "POST",
      });
      const result = await response.json();
      if (result.success) {
        ...
        Alert.alert(
          "Queue Updated",
          `Current position: #${result.data.position.toString().padStart(2, "0")}`,
        );
      }
    } catch (error) {
  ...
  const handleNearbyFacilities = async () => {
    try {
      ...
      if (result.success && result.data.url) {
        ...
        Linking.openURL(
          "https://www.google.com/maps/search/government+offices+near+me",
        );
      }
    } catch (error) {
      Linking.openURL(
  ...
  const handleNotificationClick = async (notification) => {
    if (!userId) return;
    await fetch(`${API_BASE_URL}/notifications/${userId}/${notification.id}`, {
      method: "PUT",
    });
  ...
  const onRefresh = async () => {
    ...
    await Promise.all([fetchAnnouncements(), fetchServiceArrangements()]);
    setRefreshing(false);
  };
...
{bookingDetails.all?.length || 0}
        ...
        <View style={styles.section}>
          ...
          <Text
            style={[styles.lastRefreshText, { color: colors.textSecondary }]}
          >
          ...
          >
            <View style={styles.progressHeader}>
              ...
              <View
                style={[
                  styles.liveBadge,
                  queueStatus.status === "completed" && styles.completedBadge,
                ]}
              >
            ...
            <View style={styles.connectionRow}>
              ...
              <Text style={styles.connectionText}>
                {queueStatus.totalWaiting || 0} people ahead
              </Text>
            </View>
          </LinearGradient>
          ...
          <View style={styles.queueProgress}>
            ...
            <Text style={[styles.queueHint, { color: colors.textSecondary }]}>
              {queueStatus.position > 0
                ? `Approximately ${queueStatus.estimatedTime} waiting time`
                : "Your turn is next!"}
            </Text>
          </View>
        </View>
          ...
          {serviceArrangements.map((item, index) => (
            ...
            >
              ...
              <View style={styles.serviceContent}>
                <Text style={[styles.serviceLabel, { color: item.color }]}>
                  {item.title}
                </Text>
                <Text
        ...
        >
          ...
          <Text style={[styles.nearbyText, { color: colors.textSecondary }]}>
            Find government offices, service centers near you
          </Text>
          <TouchableOpacity
            style={styles.nearbyButton}
            onPress={handleNearbyFacilities}
          >
      ...
      >
        <TouchableOpacity
          ...
          onPress={() => setActiveFooterTab("home")}
        >
          <Ionicons
            name={activeFooterTab === "home" ? "home" : "home-outline"}
            size={24}
            color={activeFooterTab === "home" ? "#1E3A8A" : "#94A3B8"}
          />
          <Text
            style={[
              styles.tabLabel,
              activeFooterTab === "home" && styles.activeTabLabel,
            ]}
          >
            Home
          </Text>
        ...
        <TouchableOpacity
          style={styles.tabItem}
          onPress={() => {
            setActiveFooterTab("booking");
            setServiceTypeModalVisible(true);
          }}
        >
          <Ionicons
            name={
              activeFooterTab === "booking" ? "calendar" : "calendar-outline"
            }
            size={24}
            color={activeFooterTab === "booking" ? "#1E3A8A" : "#94A3B8"}
          />
          <Text
            style={[
              styles.tabLabel,
              activeFooterTab === "booking" && styles.activeTabLabel,
            ]}
          >
            Booking
          </Text>
        ...
        <TouchableOpacity
          style={styles.tabItem}
          onPress={() => setActiveFooterTab("settings")}
        >
          <Ionicons
            name={
              activeFooterTab === "settings" ? "settings" : "settings-outline"
            }
            size={24}
            color={activeFooterTab === "settings" ? "#1E3A8A" : "#94A3B8"}
          />
          <Text
            style={[
              styles.tabLabel,
              activeFooterTab === "settings" && styles.activeTabLabel,
            ]}
          >
            Settings
          </Text>
        ...
        <View style={styles.modalOverlay}>
          ...
          >
            <View style={styles.modalHeader}>
              ...
              <TouchableOpacity
                onPress={() => setServiceTypeModalVisible(false)}
              >
              ...
              {services.map((service) => (
                ...
                >
                  ...
                  <View style={styles.serviceTypeInfo}>
                    <Text
                      style={[styles.serviceTypeName, { color: colors.text }]}
                    >
                      {service.name}
                    </Text>
                    <Text
      ...
      <Modal visible={queueModalVisible} transparent animationType="fade">
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            <View
              style={[styles.queueCard, { backgroundColor: colors.background }]}
            >
              ...
              <Text
                style={[
                  styles.queueCardConnection,
                  { color: colors.textSecondary },
                ]}
              >
                People Ahead: {queueStatus.totalWaiting || 0}
              </Text>
              <View style={styles.queueProgress}>
                <View style={styles.progressBarBg}>
                  <View
              ...
              {(activeBookingTab === "ongoing"
                ...
                >
                  <View style={styles.bookingIcon}>
                    <Ionicons
                      name={
                        booking.service === "license"
                          ? "car"
                          : booking.service === "passport"
                            ? "airplane"
                            : "calendar"
                      }
                      size={20}
      ...
      {/* Settings Modal */}
      <Modal
        visible={activeFooterTab === "settings"}
        transparent
        animationType="slide"
      >
        <View style={styles.modalOverlay}>
          ...
          >
            ...
            >
              ...
              <TouchableOpacity
                onPress={() =>
                  Alert.alert("Privacy Policy", "Your data is secure with us.")
                }
              >
                <Ionicons
                  name="chevron-forward"
            ...
            >
              ...
              <TouchableOpacity
                onPress={() =>
                  Alert.alert("Terms of Service", "Terms and conditions apply.")
                }
              >
                <Ionicons
                  name="chevron-forward"
