...
import { COLORS, styles } from './license-booking.styles';
...
const licenseInitialState = {
  ...
  nicNumber: "", nicIssueDate: "", 
  licenseType: "new", vehicleCategories: [], 
  medicalCertificate: "", medicalIssueDate: "", medicalExpiryDate: "",
  previousLicense: "", previousLicenseNo: "", previousIssueDate: "", previousExpiryDate: "",
  bloodGroup: "", emergencyContact: "", emergencyMobile: "", emergencyRelation: "",
  drivingSchool: "", instructorName: "",
  timeslot: "",
};
...
const ServiceSelectionScreen = memo(({ onSelect }) => (
  <View style={styles.serviceContainer}>
    <View style={styles.serviceHeader}>
      ...
      <Text style={styles.serviceSubtitle}>Select the license service you need</Text>
    </View>
    ...
    <TouchableOpacity style={styles.serviceCard} onPress={() => onSelect('new')} activeOpacity={0.8}>
      ...
      <View style={styles.serviceContent}>
        ...
        <Text style={styles.serviceDescription}>Apply for your first driving license</Text>
    ...
    <TouchableOpacity style={styles.serviceCard} onPress={() => onSelect('renewal')} activeOpacity={0.8}>
      ...
      <View style={styles.serviceContent}>
        ...
        <Text style={styles.serviceDescription}>Renew your existing or expired license</Text>
        <View style={styles.serviceBadge}>
    ...
    <TouchableOpacity style={styles.serviceCard} onPress={() => onSelect('upgrade')} activeOpacity={0.8}>
      ...
      <View style={styles.serviceContent}>
        ...
        <Text style={styles.serviceDescription}>Add new vehicle categories</Text>
        <View style={styles.serviceBadge}>
          <Ionicons name="information-circle-outline" size={14} color="#4F46E5" />
          <Text style={[styles.serviceBadgeText, { color: '#4F46E5' }]}>Heavy vehicles/Motorcycle</Text>
        </View>
      </View>
      <Ionicons name="chevron-forward" size={24} color={COLORS.textTertiary} />
    </TouchableOpacity>
    ...
    <TouchableOpacity style={styles.serviceCard} onPress={() => onSelect('lost')} activeOpacity={0.8}>
      ...
      <View style={styles.serviceContent}>
        ...
        <Text style={styles.serviceDescription}>Replace lost or damaged license</Text>
        <View style={styles.serviceBadge}>
    ...
    <TouchableOpacity style={styles.serviceCard} onPress={() => onSelect('international')} activeOpacity={0.8}>
      ...
      <View style={styles.serviceContent}>
        ...
        <Text style={styles.serviceDescription}>Apply for IDP for overseas driving</Text>
        <View style={styles.serviceBadge}>
          <Ionicons name="airplane-outline" size={14} color={COLORS.success} />
          <Text style={[styles.serviceBadgeText, { color: COLORS.success }]}>Valid for 1 year</Text>
        </View>
      </View>
      <Ionicons name="chevron-forward" size={24} color={COLORS.textTertiary} />
    </TouchableOpacity>
...
export default function LicenseBooking() {
  ...
  const toggleVehicleCategory = (category) => {
    setForm(prev => {
      ...
      if (categories.includes(category)) {
        return { ...prev, vehicleCategories: categories.filter(c => c !== category) };
      } else {
  ...
  const loadTempData = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/license-booking/temp-data/${userId}`);
  ...
  const loadRecommendedSlot = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/license-booking/recommended-slot/${userId}`);
      const result = await response.json();
  ...
  const loadAvailableSlots = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/license-booking/available-slots`);
      const result = await response.json();
  ...
  const handleDocumentUpload = () => {
    Alert.alert(
      ...
      [
        ...
        { text: "🏥 Medical Certificate", onPress: () => uploadDocumentWithPicker("medical_certificate") },
        { text: "📸 Passport Photo", onPress: () => uploadDocumentWithPicker("passport_photo") },
        { text: "🚗 Previous License", onPress: () => uploadDocumentWithPicker("previous_license") },
  ...
  const saveStep1 = async () => {
    ...
    const minAge = serviceType === 'new' ? 18 : (serviceType === 'upgrade' ? 18 : 17);
    if (age < minAge) { 
      Alert.alert("Validation Error", `Applicant must be at least ${minAge} years old for this license type`); 
      return false; 
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/license-booking/save-personal/${userId}`, { 
        method: 'POST', 
  ...
  const saveStep2 = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/license-booking/save-identity/${userId}`, { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
        body: JSON.stringify(payload) 
      });
  ...
  const saveStep3 = async () => {
    ...
    if (serviceType !== 'renewal' && serviceType !== 'international') {
      if (!form.medicalCertificate || !form.medicalIssueDate) {
        Alert.alert("Validation Error", "Medical certificate is required for new license applications");
        return false;
      }
    }
    setLoading(true);
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/license-booking/save-medical/${userId}`, { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
        body: JSON.stringify(payload) 
      });
  ...
  const saveStep4 = async () => {
    ...
    if (serviceType === 'new' || serviceType === 'upgrade') {
      if (!form.vehicleCategories || form.vehicleCategories.length === 0) {
        Alert.alert("Validation Error", "Please select at least one vehicle category");
        return false;
      }
    }
    setLoading(true);
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/license-booking/save-license-details/${userId}`, { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
        body: JSON.stringify(payload) 
      });
  ...
  const submit = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/license-booking/confirm/${userId}`, { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
  ...
  const getServiceTitle = () => {
    switch(serviceType) {
      case 'new': return 'New Driving License';
      case 'renewal': return 'License Renewal';
      case 'upgrade': return 'License Upgrade';
      case 'lost': return 'Lost/Damaged License';
      case 'international': return 'International Driving Permit';
      default: return 'License Service';
    }
  };
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.keyboardAvoid}>
        ...
        <ScrollView ref={scrollViewRef} contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false} keyboardShouldPersistTaps="handled">
          {step === 1 && (
            <View style={styles.card}>
              ...
              <View style={styles.formSection}>
                ...
                <DateInput icon="calendar-outline" label="Date of Birth" value={form.dob} onChange={(t) => updateField("dob", t)} required focusedField={focusedField} setFocusedField={setFocusedField} minAge={serviceType === 'new' ? 18 : 17} />
                <SelectInput label="Gender" value={form.gender} onChange={(t) => updateField("gender", t)} options={["Male", "Female", "Other"]} required />
                <SelectInput label="Blood Group" value={form.bloodGroup} onChange={(t) => updateField("bloodGroup", t)} options={["A+", "A-", "B+", "B-", "AB+", "AB-", "O+", "O-"]} />
                ...
                {serviceType !== 'new' && (
                  <>
                    <View style={styles.divider}>
                      <View style={styles.dividerLine} /><Text style={styles.dividerText}>Previous License Details</Text><View style={styles.dividerLine} />
                    </View>
                    <SelectInput label="Do you have previous license?" value={form.previousLicense} onChange={(t) => updateField("previousLicense", t)} options={["Yes", "No"]} required />
                    {form.previousLicense === "Yes" && (
                      <>
                        <Input icon="document-text-outline" label="Previous License Number" value={form.previousLicenseNo} onChangeText={(t) => updateField("previousLicenseNo", t)} required focusedField={focusedField} setFocusedField={setFocusedField} />
          ...
          {step === 3 && (
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <View style={styles.cardIcon}><Ionicons name="medical" size={24} color={COLORS.primary} /></View>
                <View style={styles.cardHeaderText}>
                  <Text style={styles.section}>Medical & Address Information</Text>
                  <Text style={styles.sectionSubtext}>Enter medical certificate and address details</Text>
                </View>
              </View>
              <View style={styles.formSection}>
                {serviceType !== 'renewal' && serviceType !== 'international' && (
                  <>
                    <Text style={styles.subsectionTitle}>Medical Certificate</Text>
                    <Input icon="document-text-outline" label="Medical Certificate Number" value={form.medicalCertificate} onChangeText={(t) => updateField("medicalCertificate", t)} required focusedField={focusedField} setFocusedField={setFocusedField} />
                    <View style={styles.rowFields}>
                      <View style={styles.halfField}><DateInput icon="calendar-outline" label="Issue Date" value={form.medicalIssueDate} onChange={(t) => updateField("medicalIssueDate", t)} required focusedField={focusedField} setFocusedField={setFocusedField} minAge={0} /></View>
                      <View style={styles.halfField}><DateInput icon="calendar-outline" label="Expiry Date" value={form.medicalExpiryDate} onChange={(t) => updateField("medicalExpiryDate", t)} focusedField={focusedField} setFocusedField={setFocusedField} minAge={0} /></View>
                    </View>
                    <View style={styles.infoBox}>
                      <Ionicons name="information-circle-outline" size={20} color={COLORS.primary} />
                      <Text style={styles.infoText}>Medical certificate must be from a registered medical practitioner</Text>
                    </View>
                  </>
                )}
                ...
                <View style={styles.divider}>
                  <View style={styles.dividerLine} /><Text style={styles.dividerText}>Driving School (Optional)</Text><View style={styles.dividerLine} />
                </View>
                <Input icon="school-outline" label="Driving School Name" value={form.drivingSchool} onChangeText={(t) => updateField("drivingSchool", t)} focusedField={focusedField} setFocusedField={setFocusedField} />
                <Input icon="person-outline" label="Instructor Name" value={form.instructorName} onChangeText={(t) => updateField("instructorName", t)} focusedField={focusedField} setFocusedField={setFocusedField} />
              </View>
            </View>
          )}
          ...
          {step === 4 && (
            <View style={styles.card}>
              <View style={styles.cardHeader}>
                <View style={styles.cardIcon}><Ionicons name="car" size={24} color={COLORS.primary} /></View>
                <View style={styles.cardHeaderText}>
                  <Text style={styles.section}>License & Emergency Details</Text>
                  <Text style={styles.sectionSubtext}>Select vehicle categories and emergency contact</Text>
                </View>
              </View>
              <View style={styles.formSection}>
                {(serviceType === 'new' || serviceType === 'upgrade') && (
                  <>
                    ...
                    <Text style={[styles.hintText, { marginBottom: 12 }]}>Select all categories you wish to apply for:</Text>
                    <View style={styles.selectContainer}>
                      {vehicleCategoryOptions.map((category) => (
                        <TouchableOpacity 
                          ...
                          style={[
                            styles.selectOption, 
                            form.vehicleCategories?.includes(category) && styles.selectOptionActive
                          ]} 
                          onPress={() => toggleVehicleCategory(category)}
                        >
                          <Text style={[
                            styles.selectText, 
                            form.vehicleCategories?.includes(category) && styles.selectTextActive
                          ]}>
                            {category}
                          </Text>
              ...
              <View style={styles.formSection}>
                ...
                <View style={styles.documentSection}>
                  ...
                  <TouchableOpacity style={[styles.uploadSection, isDocumentsUploaded && styles.uploadSectionCompleted]} onPress={handleDocumentUpload}>
                    ...
                    <View style={styles.uploadContent}>
                      ...
                      <Text style={styles.uploadSubtext}>NIC, Medical Certificate, Photos (required)</Text>
                    </View>
                ...
                <View style={styles.reviewCard}>
                  ...
                  <View style={styles.reviewSection}>
                    ...
                    {[["Full Name", form.fullName], ["DOB", form.dob], ["Gender", form.gender], ["Blood Group", form.bloodGroup], ["Mobile", form.mobile]].map(([l, v]) => (
                  ...
                  <View style={styles.reviewDivider} />
                  <View style={styles.reviewSection}>
                    <Text style={styles.reviewSectionTitle}>License Details</Text>
                    {[["License Type", serviceType], ["Vehicle Categories", form.vehicleCategories?.join(", ") || "—"], ["Emergency Contact", form.emergencyContact]].map(([l, v]) => (
                      <View style={styles.reviewRow} key={l}>
                        <Text style={styles.reviewLabel}>{l}</Text>
                        <Text style={styles.reviewValue}>{v || "—"}</Text>
                      </View>
