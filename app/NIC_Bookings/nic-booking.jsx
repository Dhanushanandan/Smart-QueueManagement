...
import { COLORS, styles } from './nic-booking.styles';
...
const registrationInitialState = {
  ...
  address: "", district: "", ds: "", gn: "", birthNo: "", birthDate: "", citizenship: "", timeslot: "",
};

const renewalInitialState = {
  fullName: "", fullNameLocal: "", dob: "", gender: "", mobile: "+94",
  oldNicNumber: "", oldNicIssueDate: "", reason: "lost",
  policeStation: "", complaintNumber: "", complaintDate: "",
  address: "", district: "", ds: "", gn: "", birthNo: "", birthDate: "", timeslot: "", remarks: "",
};
...
const ServiceSelectionScreen = memo(({ onSelect }) => (
  <View style={styles.serviceContainer}>
    ...
    <TouchableOpacity style={styles.serviceCard} onPress={() => onSelect('registration')} activeOpacity={0.8}>
      <View style={[styles.serviceIcon, { backgroundColor: COLORS.primaryLight }]}><Ionicons name="person-add-outline" size={32} color={COLORS.primary} /></View>
      <View style={styles.serviceContent}>
        <Text style={styles.serviceName}>New NIC Registration</Text>
        <Text style={styles.serviceDescription}>Apply for your first National Identity Card</Text>
        <View style={styles.serviceBadge}><Ionicons name="time-outline" size={14} color={COLORS.success} /><Text style={styles.serviceBadgeText}>First time applicants</Text></View>
      </View>
      <Ionicons name="chevron-forward" size={24} color={COLORS.textTertiary} />
    </TouchableOpacity>
    <TouchableOpacity style={styles.serviceCard} onPress={() => onSelect('renewal')} activeOpacity={0.8}>
      <View style={[styles.serviceIcon, { backgroundColor: '#FEF3C7' }]}><Ionicons name="sync-outline" size={32} color={COLORS.warning} /></View>
      <View style={styles.serviceContent}>
        <Text style={styles.serviceName}>NIC Renewal / Replacement</Text>
        <Text style={styles.serviceDescription}>Replace lost, damaged, or stolen NIC</Text>
        <View style={styles.serviceBadge}><Ionicons name="alert-circle-outline" size={14} color={COLORS.orange} /><Text style={[styles.serviceBadgeText, { color: COLORS.orange }]}>Lost/Damaged/Stolen</Text></View>
      </View>
      <Ionicons name="chevron-forward" size={24} color={COLORS.textTertiary} />
    </TouchableOpacity>
    <View style={styles.serviceInfo}><Ionicons name="information-circle-outline" size={20} color={COLORS.textSecondary} /><Text style={styles.serviceInfoText}>Please bring required documents to your appointment</Text></View>
  </View>
));
...
export default function NICBooking() {
  ...
  useEffect(() => {
    if ((serviceType === 'registration' && step === 3) || (serviceType === 'renewal' && step === 4)) {
  ...
  const handleServiceSelect = (type) => {
    setServiceType(type);
    setForm(type === 'registration' ? {...registrationInitialState, mobile: "+94"} : {...renewalInitialState, mobile: "+94"});
  };
  ...
  const clearAll = useCallback(() => {
    Alert.alert("Reset Form", "Are you sure you want to clear all fields?", [
      ...
      { text: "Clear All", style: "destructive", onPress: () => { 
        setForm(serviceType === 'registration' ? {...registrationInitialState, mobile: "+94"} : {...renewalInitialState, mobile: "+94"});
  ...
  const loadTempData = async () => {
    try {
      const endpoint = serviceType === 'registration' ? 'temp-data' : 'temp-renewal-data';
      const response = await fetch(`${API_BASE_URL}/nic-booking/${endpoint}/${userId}`);
  ...
  const loadRecommendedSlot = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/nic-booking/recommended-slot/${userId}`);
      const result = await response.json();
  ...
  const loadAvailableSlots = async () => {
    try {
      const response = await fetch(`${API_BASE_URL}/nic-booking/available-slots`);
      const result = await response.json();
  ...
  const saveRegistrationStep1 = async () => {
    ...
    if (age < 15) { 
      Alert.alert("Validation Error", `Applicant must be at least 15 years old`); 
      return false; 
    }
  ...
  const saveRegistrationStep2 = async () => {
    if (!form.address || !form.district || !form.ds || !form.gn || !form.birthNo || !form.birthDate || !form.citizenship) { 
      Alert.alert("Validation Error", "Please fill all required fields"); 
      return false; 
    }
    setLoading(true);
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/nic-booking/save-verification/${userId}`, { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
        body: JSON.stringify(payload) 
      });
  ...
  const submit = async () => {
    ...
    setLoading(true);
    try {
      const endpoint = serviceType === 'registration' ? 'confirm' : 'confirm-renewal';
      ...
      const response = await fetch(`${API_BASE_URL}/nic-booking/${endpoint}/${userId}`, { 
        method: 'POST', 
        headers: { 'Content-Type': 'application/json' }, 
        body: JSON.stringify({ timeslot: selectedSlot, documents: docsForAPI }) 
      });
      const result = await response.json();
      if (result.success) {
  ...
  const getProgressSteps = () => {
    if (serviceType === 'registration') return [
    ...
    <SafeAreaView style={styles.container}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === "ios" ? "padding" : undefined} style={styles.keyboardAvoid}>
        <View style={styles.header}>
          ...
          <View style={styles.headerCenter}>
            <Text style={styles.title}>{serviceType === 'registration' ? 'New NIC Registration' : 'NIC Renewal'}</Text>
            <Text style={styles.subtitle}>{serviceType === 'registration' ? 'First time application' : 'Lost/Damaged replacement'}</Text>
          </View>
              ...
              {step === 2 && (
                <View style={styles.card}>
                  ...
                  <View style={styles.formSection}>
                    ...
                    <SelectInput label="Citizenship Status" value={form.citizenship} onChange={(t) => updateField("citizenship", t)} options={["Sri Lankan by Birth", "Sri Lankan by Registration", "Dual Citizen"]} required />
                  </View>
                </View>
              )}
                    ...
                    <View style={styles.documentSection}>
                      ...
                      <TouchableOpacity style={[styles.uploadSection, isDocumentsUploaded && styles.uploadSectionCompleted]} onPress={handleDocumentUpload}>
                        ...
                        <View style={styles.uploadContent}>
                          ...
                          <Text style={styles.uploadSubtext}>Birth certificate, photos (required)</Text>
                        </View>
